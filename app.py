...
f_status = fcol1.selectbox("表示ステータス", ["すべて"] + STATUSES)
f_cat    = fcol2.selectbox("表示分類",       ["すべて"] + CATEGORIES)
...
def make_display(row):
    ...
    return pd.Series({
        ...
        "開始日": str(row["start_date"]) if pd.notnull(row["start_date"]) else "",
        "読了日": str(row["end_date"])   if pd.notnull(row["end_date"])   else "",
        "経過": f"{days}日" if days is not None else "",
        "感想": row["note"],
        "_alert": alert
    })
...
if not view.empty:
    ...
    # ⚠️がある時だけ列を出す
    cols = ["ID", "タイトル", "著者", "分類", "ステータス", "開始日", "読了日", "経過", "感想"]
    if disp["⚠️"].str.contains("⚠️").any(): cols.insert(1, "⚠️")
    
    st.dataframe(
        disp[cols].style.apply(lambda r: ["background-color: #fff0f0"]*len(r) if r.get("⚠️") else [""]*len(r), axis=1),
...
with st.expander("➕ 本を追加 / 編集する"):
    ...
    edit_sel = st.selectbox("対象を選択", options)

    if edit_sel == "新規追加":
        init = {"id": None, "title":"","author":"","category":CATEGORIES[0],"status":"未読","start_date":None,"end_date":None,"note":""}
    else:
        target_id = int(edit_sel.split(" - ")[0].replace("ID:", ""))
        init = df[df["id"] == target_id].iloc[0].to_dict()
    ...
    with st.form("book_form"):
        ...
        # 開始日と読了日を常に表示（初期値は今日）
        start_date = st.date_input("読書開始日", value=init["start_date"] if pd.notnull(init["start_date"]) else date.today())
        end_date = st.date_input("読了日 (読了時のみ有効)", value=init["end_date"] if pd.notnull(init["end_date"]) else date.today())
    ...
    if submitted:
        if not title or not author:
            ...
            # 読了じゃない場合はend_dateを保存しない（任意）
            final_end = end_date if status == "読了" else None
            
            if edit_sel == "新規追加":
                new_id = int(df["id"].max()) + 1 if not df.empty else 1
                new_row = pd.DataFrame([{"id": new_id, "title": title, "author": author, "category": cat, "status": status_en, "start_date": start_date, "end_date": final_end, "note": note}])
                df = pd.concat([df, new_row], ignore_index=True)
            else:
                idx = df[df["id"] == target_id].index[0]
                df.loc[idx, ["title","author","category","status","start_date","end_date","note"]] = [title, author, cat, status_en, start_date, final_end, note]
...
with st.expander("🗑 本を削除する"):
    ...
    del_sel = st.selectbox("削除対象", del_options)
